...
// ... AQUÍ VA TU ESTRUCTURA my_map COMPLETA ...
// (Usando my_map<int, int>)

int main() {
    // Optimización de I/O crítica para 10^6 elementos
    ...
    my_map<int, int> cubetas(2 * k + 1);

    // Calculamos el tamaño de la cubeta
    int tamano_cubeta = (t == 0) ? 1 : t + 1;

    for (int i = 0; i < n; ++i) {
        int valor_actual = arreglo[i];

        // Obtenemos el ID de la cubeta actual
        int id_actual = valor_actual / tamano_cubeta;

        // 1. Revisamos si ya hay un número en la MISMA cubeta
        if (cubetas.has_key(id_actual)) {
            cout << "SI\n";
            return 0;
        }

        // 2. Revisamos la cubeta ANTERIOR
        if (cubetas.has_key(id_actual - 1) &&
            abs(valor_actual - cubetas[id_actual - 1]) <= t) {
            cout << "SI\n";
            return 0;
            }

        // 3. Revisamos la cubeta SIGUIENTE
        if (cubetas.has_key(id_actual + 1) &&
            abs(valor_actual - cubetas[id_actual + 1]) <= t) {
            cout << "SI\n";
            return 0;
        ...
        // Guardamos el número en su cubeta correspondiente
        cubetas[id_actual] = valor_actual;

        // Mantenemos la ventana de tamaño k borrando el número que salió del rango
        if (i >= k) {
            int valor_viejo = arreglo[i - k];
            int id_viejo = valor_viejo / tamano_cubeta;
            cubetas.erase(id_viejo);
        }
    }
