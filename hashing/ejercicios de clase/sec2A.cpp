...
int main() {
    ...
    // Mapa para guardar cuántas veces aparece cada número
    my_map<int, int> conteo_frecuencias(2 * n);

    // 1. Leemos los 'n' números y contamos sus apariciones
    for (int i = 0; i < n; ++i) {
        int numero_actual;
        cin >> numero_actual;

        // Sumamos 1 a la cantidad de veces que ha aparecido este número
        conteo_frecuencias[numero_actual]++;
    }

    bool es_primer_duplicado = true;

    // 2. Revisamos los números del 1 al total_elementos en orden creciente
    for (int numero = 1; numero <= n; ++numero) {

        // Si el número existió en el mapa y apareció exactamente 2 veces
        if (conteo_frecuencias.has_key(numero) && conteo_frecuencias[numero] == 2) {

            if (!es_primer_duplicado) {
                cout << " "; // Imprime un espacio antes si no es el primer número
            }

            cout << numero;
            es_primer_duplicado = false;
        }
    }
    cout << "\n";
    return 0;
